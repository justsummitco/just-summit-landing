...
test('Home Page shows the trust-focused hardware presale funnel', async ({ page }) => {
  ...
  await expect(page.getByText(/Concept render · Product design in development/i)).toBeVisible()
  ...
  await expect(page.locator('#how-it-works').getByText('Concept render', { exact: true })).toBeVisible()
  ...
  await expect(page.getByTestId('hero-product-visual')).toBeVisible()
